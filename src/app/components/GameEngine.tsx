...
import MiningEntrance from './MiningEntrance';
...
function BetInput({ balance, onStartGame }: BetInputProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center flex-1 w-full gap-4 px-2">
      <div className="w-full rounded-2xl p-5 relative" style={{ background: 'rgba(0,0,0,0.8)', border: '1px solid rgba(59,130,246,0.5)', boxShadow: '0 0 60px rgba(59, 130, 246, 0.7), inset 0 0 35px rgba(59, 130, 246, 0.25)', animation: 'pulse-glow 15s infinite ease-in-out' }}>
        ...
        <div className="space-y-6 mb-2">
          <div className="flex flex-col gap-4">
            ...
            <input
              ...
              max={maxBet}
              step={5}
              value={balance === 0 ? 5 : betStr}
              disabled={balance === 0}
              onChange={e => {
                setBetStr(e.target.value);
                setError('');
                localStorage.setItem('rtb-last-bet', e.target.value);
              }}
              className={`w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-green-500 ${balance === 0 ? 'opacity-20' : ''}`}
              style={{
...
export default function GameEngine() {
  ...
  return (
    <div className="flex flex-col felt-bg relative" style={{ height: '100dvh', maxHeight: '100dvh' }}>
      ...
      <main className="flex flex-col items-center justify-center px-4 pb-3 pt-2 max-w-lg mx-auto w-full relative" style={{ flex: 1, minHeight: 0 }}>
        <MiningEntrance balance={balance} stage={stage} />
        {stage === 'idle' ? (
